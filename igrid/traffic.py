...
class Traffic(IGRID):
    ...
    def __create_UDP_packet__(self, src, dst, port, payload):
        return Ether()/IP(src=src, dst=dst)/UDP(dport=port)/payload
    ...
    def __unpack_send_TCP_packet__(self, args):
        self.__send_TCP_packet__(*args)

    def __batch_schedule__(self, batch=[], interval=1, dst={}):
        for node in batch:
            schedule.every(interval=interval).seconds.do(
                job_func=self.__send_UDP_packet__, src=str(node.IP()), dst=dst).tag("sim")
    ...
    def send_once(self, port=8080):
        ...
        pool.map(self.__unpack_send_TCP_packet__, itertools.izip(
